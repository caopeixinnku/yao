...
var DialogView = Backbone.View.extend({
    ...
    initialize: function (options) {
        ...
        // track when a file dialog is open
        this._fileDialogActive = false;
    ...
    renderSettingsView: function () {
        if (!this.genableDialogView) {
            this.genableDialogView = new GenableDialogView({ el: Backbone.$(".dialog-settings"), collection: this.generatorModel.layerCollection});
        }
    ...
    handleDocKeyUp: function (e) {
        ...
        // key presses from inside a select element should be ignored
        if ($target.is("select")) {
            return;
        }

        // ESC and ENTER keyups immediately following the file/folder dialog dismissal
        // should be ignored
        if ((e.which === 27 || e.which === 13) && this._fileDialogActive) { // esc || enter
            CremaGlobal.window.console.debug("IGNORING %s key because OS file dialog is (or was recently) open",
                e.which === 27 ? "ESC" : "Enter");
            return;
        }
    ...
    cancelDialog: function () {
        CremaGlobal.window.console.debug("CLOSING dialog via dialogView.cancelDialog()");
    ...
    updateQuickExportComponentPaths: function(quickExportComponents) {
        ...
        if (quickExportComponents.length === 1) {
            ...
            fileDialogPromise = FileDialog.promptForExportFile(docFileBaseName, component.basename, component.extension, docFileDirectory)
                .then(function(filepath) {
                    ...
                    return Q.delay(DIALOG_STATE_DELAY).then(() => Q.reject(err));
                });
        } else {
            fileDialogPromise = FileDialog.promptForExportDirectory(docFileBaseName, docFileDirectory)
        ...
        // 500ms delay to allow keyboard handling to properly ignore the esc/enter key from the dialog
        return fileDialogPromise.delay(DIALOG_STATE_DELAY).finally(function () {
            this._fileDialogActive = false;
        }.bind(this));
    },
    ...
    extractLayerAssets: function () {
        ...
        var docId = this.generatorModel.get("docId"),
            ...
            handleMetadata = function (result) {
                ...
                if (numExported) {
                    // Check if there were requests to include copyright and contact info metadata
                    var includeUserMetadata = _.some(quickExportComponents, hasMetadata);
                    return Metadata.readMetadata(includeUserMetadata)
                        .then(writeAllMetadata)
                        .catch(function (e) {
                            CremaGlobal.window.console.warn('read/write metadata ERROR', e);
