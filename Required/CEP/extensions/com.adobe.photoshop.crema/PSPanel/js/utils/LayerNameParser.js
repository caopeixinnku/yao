...
(function() {
  ...
  function parseMapped(input) {
      var ret = parse(input);
      
      if (ret && ret.length) {
          ret.forEach(function (component) {
              if (component.extension === "jpeg") {
                  component.extension = "jpg";
                  if (component.name) {
                    component.name = component.name.replace(/\.jpeg$/, ".jpg");
                  }
                  if (component.file) {
                    component.file = component.file.replace(/\.jpeg$/, ".jpg");
                  }
              }
              
              //conflate file and folder because the rest of our code already does
              if (component.folder && component.folder.length) {
                  component.file = component.folder.concat(component.file).join("/");
