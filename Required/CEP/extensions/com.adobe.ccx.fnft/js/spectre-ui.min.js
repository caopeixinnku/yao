...
window.componentHandler = function() {
    ...
    function n(t, s) {
        if (void 0 === t && void 0 === s) for (var o = 0; o < h.length; o++) n(h[o].className, h[o].cssClass); else {
            ...
            for (var a = document.querySelectorAll("." + s), c = 0; c < a.length; c++) i(a[c], r);
        }
    }
    function i(n, i) {
        if (!("object" == typeof n && n instanceof Element)) throw new Error("Invalid argument provided to upgrade SPC element.");
        var o = t(n), r = [];
        if (i) s(n, i) || r.push(e(i)); else {
            var l = n.classList;
            h.forEach(function(e) {
                l.contains(e.cssClass) && -1 === r.indexOf(e) && !s(n, e.className) && r.push(e);
            });
        }
        for (var a, u = 0, _ = r.length; _ > u; u++) {
            if (a = r[u], !a) throw new Error("Unable to find a registered component for the given class.");
            o.push(a.className), n.setAttribute("data-upgraded", o.join(","));
            var d = new a.classConstructor(n);
            d[p] = a, c.push(d);
            for (var m = 0, C = a.callbacks.length; C > m; m++) a.callbacks[m](n);
            a.widget && (n[a.className] = d);
            var S = document.createEvent("Events");
            S.initEvent("spc-componentupgraded", !0, !0), n.dispatchEvent(S);
        }
    }
    function o(e) {
...
SpectreDropdown.prototype.CssClasses_ = {
    ...
    e -= 70;
    var n, i = s.scrollTop, o = e - i, r = 0, l = 20;
    t = "undefined" == typeof t ? 500 : t;
    var a = function() {
        r += l, n = this.easeInOutQuad_(r, i, o, t), s.scrollTop = n, t > r && requestAnimationFrame(a);
    }.bind(this);
    a();
...
var SpectrePopupMenu = function(e) {
    this.element_ = e, this.init();
};

SpectrePopupMenu.prototype.CssClasses_ = {
    IS_DISABLED: "is-disabled",
    IS_OPEN: "is-open",
    IS_CHECKED: "is-checked",
    IS_FOCUS: "is-focus",
    SELECTED: "spc-popup__selected",
    OPTION: "spc-popup-list__item",
    LIST: "spc-popup-list",
    OVERLAY: "spc-popup-overlay",
    OPTIONS: "spc-popup__options",
    CONTAINER: "spc-popup-container"
}, SpectrePopupMenu.prototype.Keycodes_ = {
    ENTER: 13,
    TAB: 9,
    UP_ARROW: 38,
    DOWN_ARROW: 40,
    SPACE_BAR: 32,
    ESC: 27
}, SpectrePopupMenu.prototype.onClick_ = function(e) {
    this.addToDom();
}, SpectrePopupMenu.prototype.onOverlayClick_ = function(e) {
    this.removeFromDom();
}, SpectrePopupMenu.prototype.onListClick_ = function(e) {
    if (!this.isDisabled() && !e.target.classList.contains(this.CssClasses_.LIST)) {
        var t = this.findAncestor_(e.target, this.CssClasses_.OPTION);
        t && (this.checkElement(t), this.removeFocus(), this.currentIndex_ = this.getIndex(e.target)), 
        this.removeFromDom();
    }
}, SpectrePopupMenu.prototype.isDisabled = function() {
    return this.element_.classList.contains(this.CssClasses_.IS_DISABLED);
}, SpectrePopupMenu.prototype.toggleState = function() {
    this.element_.dispatchEvent(new Event("toggle", {
        bubbles: !0
    })), this.isOpen_ ? (this.removeFromDom(), this.element_.dispatchEvent(new Event("closed", {
        bubbles: !0
    }))) : (this.addToDom(), this.element_.dispatchEvent(new Event("opened", {
        bubbles: !0
    })));
}, SpectrePopupMenu.prototype.checkElement = function(e) {
    var t = this.containerElement_.querySelector("." + this.CssClasses_.IS_CHECKED);
    t && t.classList.remove(this.CssClasses_.IS_CHECKED), e.classList.add(this.CssClasses_.IS_CHECKED), 
    e.dispatchEvent(new Event("change", {
        bubbles: !0
    })), e.dispatchEvent(new Event("checked", {
        bubbles: !0
    })), this.element_.dispatchEvent(new CustomEvent("change", {
        detail: {
            element: e
        },
        bubbles: !0
    }));
}, SpectrePopupMenu.prototype.focusElement = function(e) {
    this.removeFocus(), e.classList.add(this.CssClasses_.IS_FOCUS);
}, SpectrePopupMenu.prototype.removeFocus = function() {
    var e = this.containerElement_.querySelector("." + this.CssClasses_.IS_FOCUS);
    e && e.classList.remove(this.CssClasses_.IS_FOCUS);
}, SpectrePopupMenu.prototype.init = function() {
    this.element_ && (this.isOpen_ = !1, this.sourceList_ = this.element_.querySelector("." + this.CssClasses_.OPTIONS), 
    this.overlayElement_ = document.createElement("div"), this.overlayElement_.classList.add(this.CssClasses_.OVERLAY), 
    this.containerElement_ = document.createElement("div"), this.containerElement_.classList.add(this.CssClasses_.CONTAINER), 
    this.element_.dataset.container ? this.parentElement_ = document.getElementById(this.element_.dataset.container) : this.parentElement_ = this.element_.parentNode, 
    this.selectedElement_ = this.element_.querySelector("." + this.CssClasses_.SELECTED), 
    this.inputElements_ = this.element_.querySelectorAll("." + this.CssClasses_.OPTION), 
    this.sourceListObserver_ = new MutationObserver(function(e) {
        e.forEach(function(e) {
            this.containerElement_.innerHTML = this.sourceList_.innerHTML, this.inputElements_ = this.containerElement_.querySelectorAll("." + this.CssClasses_.OPTION), 
            this.updateClasses_();
        }.bind(this));
    }.bind(this)), this.sourceListObserver_.observe(this.sourceList_, {
        childList: !0,
        subtree: !0
    }), this.boundInputOnChange = this.onChange_.bind(this), this.boundElementOnListClick = this.onListClick_.bind(this), 
    this.boundElementOnClick = this.onClick_.bind(this), this.boundElementOnOverlayClick = this.onOverlayClick_.bind(this), 
    this.boundElementOnKeyDown = this.onKeyDown_.bind(this), this.boundElementOnFocus = this.onFocus_.bind(this), 
    this.containerElement_.addEventListener("click", this.boundElementOnListClick), 
    this.element_.addEventListener("click", this.boundElementOnClick), this.overlayElement_.addEventListener("click", this.boundElementOnOverlayClick), 
    this.containerElement_.addEventListener("change", this.boundInputOnChange), this.element_.addEventListener("keydown", this.boundElementOnKeyDown), 
    this.containerElement_.addEventListener("keydown", this.boundElementOnKeyDown), 
    this.element_.addEventListener("focus", this.boundElementOnFocus, !0), this.element_.classList.add("is-upgraded"), 
    this.updateClasses_(), this.populateList(), this.currentIndex_ = this.getInitialIndex_());
}, SpectrePopupMenu.prototype.onKeyDown_ = function(e) {
    0 === this.inputElements_.length || this.isDisabled() || (e.keyCode === this.Keycodes_.DOWN_ARROW ? (this.isOpen_ ? (this.currentIndex_++, 
    this.currentIndex_ >= this.inputElements_.length && (this.currentIndex_ = 0), this.focusElement(this.inputElements_[this.currentIndex_]), 
    this.checkElement(this.inputElements_[this.currentIndex_]), this.scrollTo_(this.inputElements_[this.currentIndex_].offsetTop, 250, this.containerElement_.querySelector("." + this.CssClasses_.LIST))) : this.toggleState(), 
    e.preventDefault(), e.stopPropagation()) : e.keyCode === this.Keycodes_.UP_ARROW ? (this.isOpen_ ? (this.currentIndex_--, 
    this.currentIndex_ < 0 && (this.currentIndex_ = this.inputElements_.length - 1), 
    this.focusElement(this.inputElements_[this.currentIndex_]), this.checkElement(this.inputElements_[this.currentIndex_]), 
    this.scrollTo_(this.inputElements_[this.currentIndex_].offsetTop, 250, this.containerElement_.querySelector("." + this.CssClasses_.LIST))) : this.toggleState(), 
    e.preventDefault(), e.stopPropagation()) : e.keyCode === this.Keycodes_.ENTER ? this.isOpen_ && (this.checkElement(this.inputElements_[this.currentIndex_]), 
    this.toggleState(), e.preventDefault(), e.stopPropagation()) : e.keyCode === this.Keycodes_.ESC ? this.isOpen_ && (this.toggleState(), 
    e.preventDefault(), e.stopPropagation()) : e.keyCode === this.Keycodes_.SPACE_BAR && (this.isOpen_ && this.checkElement(this.inputElements_[this.currentIndex_]), 
    this.isDisabled() || this.toggleState(), -1 === this.currentIndex_ && (this.currentIndex_ = 0), 
    this.focusElement(this.inputElements_[this.currentIndex_]), e.preventDefault(), 
    e.stopPropagation()));
}, SpectrePopupMenu.prototype.onChange_ = function(e) {
    this.updateClasses_();
}, SpectrePopupMenu.prototype.onFocus_ = function(e) {}, SpectrePopupMenu.prototype.updateClasses_ = function() {
    var e, t;
    for (e = 0, t = this.inputElements_.length; t > e; e++) this.inputElements_[e].classList.contains(this.CssClasses_.IS_CHECKED) && (this.selectedElement_.innerHTML = this.inputElements_[e].innerHTML);
}, SpectrePopupMenu.prototype.getInitialIndex_ = function() {
    var e, t;
    for (e = 0, t = this.inputElements_.length; t > e; e++) if (this.inputElements_[e].classList.contains(this.CssClasses_.IS_CHECKED)) return e;
    return 0;
}, SpectrePopupMenu.prototype.easeInOutQuad_ = function(e, t, s, n) {
    return e /= n / 2, 1 > e ? s / 2 * e * e + t : (e--, -s / 2 * (e * (e - 2) - 1) + t);
}, SpectrePopupMenu.prototype.scrollTo_ = function(e, t, s) {
    e -= 140;
    var n, i = s.scrollTop, o = e - i, r = 0, l = 20;
    t = "undefined" == typeof t ? 500 : t;
    var a = function() {
    ...
    a();
}, SpectrePopupMenu.prototype.findAncestor_ = function(e, t) {
    for (;e; ) {
        if (e.classList.contains(t)) return e;
        if (e === this.element_ || e === document.body) return null;
        e = e.parentNode;
    }
    return null;
}, SpectrePopupMenu.prototype.getIndex = function(e) {
    this.inputElements_ = this.containerElement_.querySelectorAll("." + this.CssClasses_.OPTION);
    var t, s;
    for (t = 0, s = this.inputElements_.length; s > t; t++) if (this.inputElements_[t] === e) return t;
    return -1;
}, SpectrePopupMenu.prototype.removeFromDom = function() {
    return this.parentElement_.removeChild(this.overlayElement_), this.parentElement_.removeChild(this.containerElement_), 
    this.element_.focus(), this.isOpen_ = !1, null;
}, SpectrePopupMenu.prototype.addToDom = function() {
    return this.parentElement_.appendChild(this.overlayElement_), this.parentElement_.appendChild(this.containerElement_), 
    this.containerElement_.tabIndex = 0, this.containerElement_.focus(), this.isOpen_ = !0, 
    this.inputElements_ = this.containerElement_.querySelectorAll("." + this.CssClasses_.OPTION), 
    this.scrollTo_(this.inputElements_[this.currentIndex_].offsetTop, 100, this.containerElement_.querySelector("." + this.CssClasses_.LIST)), 
    null;
}, SpectrePopupMenu.prototype.populateList = function() {
    return this.containerElement_.innerHTML = this.element_.querySelector("." + this.CssClasses_.OPTIONS).innerHTML, 
    null;
}, SpectrePopupMenu.prototype.spcDowngrade_ = function() {
    this.containerElement_.removeEventListener("click", this.boundElementOnListClick), 
    this.element_.removeEventListener("click", this.boundElementOnClick), this.overlayElement_.removeEventListener("click", this.boundElementOnOverlayClick), 
    this.containerElement_.removeEventListener("change", this.boundInputOnChange), this.containerElement_.removeEventListener("keydown", this.boundElementOnKeyDown), 
    this.element_.removeEventListener("keydown", this.boundElementOnKeyDown), this.element_.removeEventListener("focus", this.boundElementOnFocus, !0), 
    this.sourceListObserver_.disconnect();
}, componentHandler.register({
    constructor: SpectrePopupMenu,
    classAsString: "SpectrePopupMenu",
    cssClass: "spc-js-popup-menu",
    widget: !0
});
...
//# sourceMappingURL=spectre-ui.min.js.map
